...
pub fn subcommand_sepatch(cli: &SepatchCli) -> Result<()> {
    ...
    // On some kernels, anon_inode permissions for untrusted_app are granted
    // through attribute-based rules (e.g. allow domain self:anon_inode), which
    // copy_avtab_rules misses because the avtab keys use the attribute ID, not
    // the type ID. Explicitly grant them so ART's userfaultfd GC works.
    if let Some(c_anon_inode) = pdb.get_class_id("anon_inode") {
        if let Some(p_anon_inode_create) = pdb.get_perm_id(c_anon_inode, "create") {
            pdb.set_rule(t_target, t_target, c_anon_inode, p_anon_inode_create, RuleAction::Allow);
        }
        if let Some(p_anon_inode_ioctl) = pdb.get_perm_id(c_anon_inode, "ioctl") {
            pdb.set_rule(t_target, t_target, c_anon_inode, p_anon_inode_ioctl, RuleAction::Allow);
            if let Some((_, t_target_uffd)) = t_uffd {
                pdb.set_rule(t_target_uffd, t_target_uffd, c_anon_inode, p_anon_inode_ioctl, RuleAction::Allow);
            }
        }
    }
