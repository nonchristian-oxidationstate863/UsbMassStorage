...
fn receive_fds(stream: &mut UnixStream, num_fds: usize) -> io::Result<Vec<OwnedFd>> {
    ...
    let ret = rustix::net::recvmsg(
        stream,
        &mut [IoSliceMut::new(&mut data_byte)],
        &mut cmsg_buf,
        RecvFlags::WAITALL,
    )?;
    tracing::debug!("receive_fds: bytes={}, data=0x{:02x}", ret.bytes, data_byte[0]);
    if ret.bytes == 0 {
    ...
    let Some(msg) = iter.next() else {
        tracing::error!("receive_fds: no ancillary message, flags={:?}", ret.flags);
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("Ancillary data has no message (bytes={}, data=0x{:02x})", ret.bytes, data_byte[0]),
        ));
    };
...
impl FromSocket for SetMassStorageRequest {
    fn from_socket(stream: &mut UnixStream) -> io::Result<Self> {
        ...
        tracing::debug!("SetMassStorageRequest: num_devices={num_devices}");
        let mut devices = vec![];

        for i in 0..num_devices {
            tracing::debug!("SetMassStorageRequest: receiving device {i}");
            let device = MassStorageDevice::from_socket(stream)?;
            tracing::debug!("SetMassStorageRequest: device {i} fd={}, cdrom={}, ro={}", device.fd.as_raw_fd(), device.cdrom, device.ro);
            devices.push(device);
        }
